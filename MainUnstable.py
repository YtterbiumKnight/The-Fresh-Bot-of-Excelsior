...
#rss commands end
        ...
        if message.author.id != message.server.owner.id: #If not server owner
            await client.send_message(message.channel, 'This command is only for the server owner and whoever they allow')
            return
        ...
        if message.author.id != message.server.owner.id: #Temporary until pings can be disabled
